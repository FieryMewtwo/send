## Requirements
This document describes how to do a full deployment of Drip on your own Linux server. You will need:
...
* First, remove that htdocs folder - we will replace it with Drip's version now
* git clone https://github.com/mozilla/drip.git htdocs
...
## Running
To have a permanently running version of Drip as a background process:
...
Now the Drip backend should be running on port 1443. You can check with:
* telnet localhost 1443

## Reverse Proxy
Of course, we don't want to expose the service on port 1443. Instead we want our normal webserver to forward all requests to Firefox drip ("Reverse proxy").
